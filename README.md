...
This application is going to connect them and promote trust across the network. It is going to garatee consumers about foods origin and freshness, but also garantee produceders that their names, values and investments are not improperly used by anyone.
Moreover, also the distribution companies will be garanteed regarding the products that they distribute.
For this reason we want to provide a platform where farmers can record their productions and issue for each of the package a unique code(TOKEN) with place of origin, date of packaging & date of shipment.
...
The food is the new starting point to reform our world and buy a food from a local producer that dosn't use any pesticides and that send or prefer to send and produce for a local market will help this change.
...
[![Business process]](https://github.com/marcofabbian/OriginalFood/blob/main/diagrams/use_cases.png)

## IV. Use Cases
* Actor registration : Farmer, Transporter & Merchant
[![Registration use case](https://github.com/marcofabbian/OriginalFood/blob/main/diagrams/use_case_actor_registration.png)]
